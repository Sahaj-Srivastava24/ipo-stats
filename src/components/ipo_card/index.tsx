...
export default function IPOCard({ipo, cls}: {ipo: IPO, cls?: string}) {
  ...
  return (
    <Link href={{pathname: ROUTES.IPO, query: {id: ipo.id}}} className={cls}>
      <Container
        css={{bg: '$ipoCardBackground', minHeight: '550px', justifyContent: 'space-between'}}
        className={`rounded-20 pt-[60px] flex md:flex-col pb-50 gap-20 hover:transition-[box-shadow] hover:duration-[200ms] hover:shadow-[0px_64px_64px_-48px_rgba(15,15,15,0.1)]`}>
        <div>
          <div className="flex flex-cols justify-between items-center mb-30 min-h-[120px] relative bg-image-bg rounded-[15px]">
